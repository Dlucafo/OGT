...
async function default_1(source) {
    ...
    try {
        ...
        if (resourceQuery === "?server-entry") {
            const { code, map } = await markoCompiler.compile(get_asset_code_1.default(resourcePath, pluginOptions.runtimeId, compiler.options.output.publicPath), resourcePath.replace(/\.marko$/, "-server-entry.marko"), baseConfig);
    ...
    if (ctx._compiler.watchMode) {
        ...
        if (missingDeps.length) {
            ...
            result += `\nrequire.context(".", false, /\\/${templateFileName === "index"
                ? ""
                : `${escape_string_regexp_1.default(templateFileName)}\\.`}(?:${missingDeps.join("|")})\\.[^d]\\w*$/)`;
        }
    }
